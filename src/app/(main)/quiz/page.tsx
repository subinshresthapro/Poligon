...
export default function QuizPage() {
  ...
  const handleAnswer = useCallback(
    (questionId: string, value: ScoreValue) => {
  ...
  const allCurrentAnswered =
    currentAnsweredCount === currentCategory.questions.length;
    ...
    <div className="min-h-screen bg-slate-50 py-8 px-4 sm:px-6">
      <div className="max-w-2xl mx-auto">
          ...
          <div className="flex gap-1.5 mt-3 flex-wrap">
            {CATEGORIES.map((cat, i) => {
              ...
              return (
                ...
                >
                  {i < categoryIndex || catAnswered ? "✓" : i + 1}
                </button>
              );
            })}
          </div>
        ...
        <p className="text-center text-xs text-slate-400 mt-4">
          You must answer all questions in this category before continuing.
        </p>
      </div>
    </div>
  );
